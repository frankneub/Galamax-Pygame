...
class SoundManager:
    ...
    def __init__(self):
        ...
        self._beat_idx    = 0
        self._beat_timer  = 0
        self._beat_interval = 40  # frames between beats
    ...
    def _buf(cls, samples):
        """Convert list of floats [-1,1] to a pygame.mixer.Sound."""
    ...
    def _make_player_die(self):
        s1 = self._sweep(600, 80, 0.5, 0.8)
        n  = self._noise(0.5, 0.4)
        return self._buf(self._mix(s1, n))
    ...
    def start_wave_beat(self, level, assets_dir="assets/audio"):
        ...
        for name in order:
            beat = self.middle_track if name in middle_names else self._load_audio_file(name, assets_dir=assets_dir)
            if beat is None:
                continue
            beat.set_volume(1.0)
    ...
    def update_beat(self, enemy_count, total_enemies):
        """Pulse the classic formation beat; tempo rises as enemies die."""
    ...
    def update(self, fdx, level):
        ...
        if self.diving:
            if self._path_idx >= len(self._path):
                # gone off-screen → re-enter from top
                self.diving    = False
                self.returning = True
                self.x         = float(self.home_x)
                self.y         = -50.0
            ...
            # slide x toward current formation position
...
class Player:
    ...
    def _get_sprite(cls, size):
        ...
        path = os.path.join("assets", "sprites", "player.jpeg")
        if not os.path.exists(path):
...
class Game:
    ...
    def _load_start_screen(self):
        path = os.path.join("assets", "backgrounds", "start_screen.png")
        if not os.path.exists(path):
            return None
        try:
    ...
    def _init_level(self, keep_lives=True):
        ...
        self._flash_alpha = 0
        self._flash_decay = 0
        bg_path = os.path.join("assets", "backgrounds", f"galaxy{self.level}.jpg")
        try:
        ...
        for ext in (".jpeg", ".jpg", ".png"):
            candidate = os.path.join("assets", "sprites", f"enemy{self.level}{ext}")
            if os.path.exists(candidate):
                enemy_path = candidate
                break
        if enemy_path:
            try:
                ...
                self._enemy_sprite = None
                self._enemy_sprite_by_size = None
        mothership_path = None
        for ext in (".png", ".jpeg", ".jpg"):
            candidate = os.path.join("assets", "sprites", f"mothership{self.level}{ext}")
            if os.path.exists(candidate):
                mothership_path = candidate
                break
        if mothership_path:
            try:
    ...
    def _try_enemy_shoot(self):
        ...
        if not alive:
            return
        # Prefer enemies that are not too far from player horizontally
